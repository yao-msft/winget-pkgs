...
3. Run `cat "/tmp/gh-aw/validation-checks.json"`. Require the recorded PR number and full head SHA to
   ...
   validation `OperationId` as the completed Check whose output explicitly reports the active
   domain-validation condition. Use `completionCheck` only to confirm the operation and labels. If
   evidence is unavailable, stale, generic, or conflicting, emit `noop`.
